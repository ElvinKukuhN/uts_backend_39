...
class User39Controller extends Controller
{
    public function dashboardPage39()
    {
        ...
        $getUser = $client->request('GET', "{$API_URL}/user39")->getBody()->getContents();
    ...
    public function profilePage39()
    {
        ...
        $getUser = $client->request('GET', "{$API_URL}/user39/{$user->id}")->getBody()->getContents();
        $getAgama = $client->request('GET', "{$API_URL}/agama39")->getBody()->getContents();
    ...
    public function detailPage39(Request $request, $id)
    {
        ...
        $client = new Client();
        $getUser = $client->request('GET', "{$API_URL}/user39/{$id}")->getBody()->getContents();
        $user = json_decode($getUser, true)['data'];
    ...
    public function putUserDetail39(Request $request)
    {
        ...
        $putUser = $client->request('PUT', "{$API_URL}/user39/{$user->id}", [
            'json' => [
                'name' => $request->name,
                'email' => $request->email,
    ...
    public function putUserAgama39(Request $request, $id)
    {
        ...
        $putUserAgama = $client->request('PUT', "{$API_URL}/user39/{$id}/agama", [
            'json' => [
    ...
    public function putUserStatus39(Request $request, $id)
    {
        ...
        $putUserStatus = $client->request('PUT', "{$API_URL}/user39/{$id}/status", [
            'json' => [
    ...
    public function putUserPassword39(Request $request)
    {
        ...
        $putUserPassword = $client->request('PUT', "{$API_URL}/user39/{$user->id}/password", [
            'json' => [
    ...
    public function putUserPhoto39(Request $request)
    {
        ...
        $putUserPhoto = $client->request('PUT', "{$API_URL}/user39/{$id}/photo", [
            'headers' => [
    ...
    public function putUserPhotoKTP39(Request $request)
    {
        $API_URL = env('API_URL', "http://localhost/BackEnd/UAS-Backend/public/api");
        ...
        $putUserPhoto = $client->request('PUT', "{$API_URL}/user39/{$id}/photoKTP", [
            'headers' => [
                'Accept' => 'application/json',
            ],
            'json' => [
    ...
    public function deleteUser39($id)
    {
        ...
        $deleteUser = $client->request('DELETE', "{$API_URL}/user39/{$id}", [
            'headers' => [
                'Accept' => 'application/json',
            ],
        ])->getBody()->getContents();
